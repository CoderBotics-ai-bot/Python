...
def hill_climbing(
    ...
    while iterations < max_iter:
        ...
        next_state = get_next_state(
            current_state, visited, find_max, max_x, min_x, max_y, min_y
        )

        if next_state is None:
            break  # no improvement is possible, terminate the loop

        current_state = next_state  # update to the next best state

    if visualization:
        from matplotlib import pyplot as plt

        scores = [s.score() for s in visited]
...
def get_next_state(
    current_state: "SearchProblem",
    visited: Set["SearchProblem"],
    find_max: bool,
    max_x: float,
    min_x: float,
    max_y: float,
    min_y: float,
) -> Optional["SearchProblem"]:
    current_score = current_state.score()
    eligible_neighbors = [s for s in current_state.get_neighbors() if s not in visited]
    bounded_neighbors = [
        s for s in eligible_neighbors if min_x <= s.x <= max_x and min_y <= s.y <= max_y
    ]

    if not bounded_neighbors:
        return None

    neighbors_and_changes = [(s, s.score() - current_score) for s in bounded_neighbors]
    if find_max:
        return max(neighbors_and_changes, key=lambda x: x[1])[0]
    else:
        return min(neighbors_and_changes, key=lambda x: x[1])[0]
