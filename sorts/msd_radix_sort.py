...
def _msd_radix_sort_inplace(
    ...
    while i <= j:
        i = find_next_zero_bit(list_of_ints, bit_position, i)
        j = find_next_one_bit(list_of_ints, bit_position, j)
...
def find_next_zero_bit(
    list_of_ints: list[int], bit_position: int, current_position: int
) -> int:
    """Find the next position where the binary bit is zero"""
    while (list_of_ints[current_position] >> bit_position) & 1:
        current_position += 1
    return current_position


def find_next_one_bit(
    list_of_ints: list[int], bit_position: int, current_position: int
) -> int:
    """Find the next position where the binary bit is one"""
    while not (list_of_ints[current_position] >> bit_position) & 1:
        current_position -= 1
    return current_position
