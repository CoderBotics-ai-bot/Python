...
class Matrix:
    ...
    def __init__(self, rows: list[list[int]]):
        ...
        Args:
        rows (list[list[int]]): The rows of the matrix. Each row is represented by a list of integers.

        Raises:
        TypeError: If rows do not represent a matrix.

        """

        self.validate_matrix(rows)
    ...
    def validate_matrix(self, rows: list[list[int]]) -> None:
        """
        Validate matrix for initialization.
        ...
        for row in rows:
            self.validate_row(row, expected_row_length, error)

    def validate_row(
        self, row: list[int], expected_length: int, error: TypeError
    ) -> None:
        """
        Validate individual matrix row during initialization.

        Args:
        row (list[int]): Individual matrix row.
        expected_length (int): The expected row length.
        error (TypeError): The error to be raised in case of invalid input.

        Raises:
        TypeError: If row is not valid.
