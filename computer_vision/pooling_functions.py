...
def avgpooling(arr: np.ndarray, size: int, stride: int) -> np.ndarray:
    """
    This function performs average pooling on a 2D array.
    """
    if arr.shape[0] != arr.shape[1]:
        raise ValueError("Input array must be square.")

    def get_avg_patch(pooling_patch: np.ndarray) -> np.ndarray:
        """
        Helper function that computes the average of a patch.
        """
        return np.average(pooling_patch)

    output_dim = ((arr.shape[0] - size) // stride) + 1
    output = np.zeros((output_dim, output_dim))

    output = np.array(
        [
            [
                get_avg_patch(arr[i : i + size, j : j + size])
                for j in range(0, arr.shape[0] - size + 1, stride)
            ]
            for i in range(0, arr.shape[1] - size + 1, stride)
        ]
    )
